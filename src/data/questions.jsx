const data = [
  {
  question: "Qual o maior planeta do Sistema Solar?",
  options: ["Marte", "Júpiter", "Saturno", "Vênus"],
  answer: "Júpiter",
  },
  {
  question: "Quem escreveu a obra 'Memórias Póstumas de Brás Cubas'?",
  options: ["Machado de Assis", "Clarice Lispector", "Carlos Drummond de Andrade", "Jorge Amado"],
  answer: "Machado de Assis",
  },
  {
  question: "Qual é o país com a maior área territorial do mundo?",
  options: ["Estados Unidos", "China", "Brasil", "Rússia"],
  answer: "Rússia",
  },
  {
  question: "Quem pintou a obra 'A Última Ceia'?",
  options: ["Michelangelo", "Rafael", "Leonardo da Vinci", "Vincent van Gogh"],
  answer: "Leonardo da Vinci",
  },
  {
  question: "Qual é o elemento químico com o símbolo 'H'?",
  options: ["Hélio", "Hidrogênio", "Carbono", "Oxigênio"],
  answer: "Hidrogênio",
  },
  {
  question: "Qual foi o primeiro país a sediar uma Copa do Mundo de futebol?",
  options: ["Brasil", "Alemanha", "Uruguai", "Argentina"],
  answer: "Uruguai",
  },
  {
  question: "Quem é o criador do personagem Harry Potter?",
  options: ["Stephenie Meyer", "J.K. Rowling", "Suzanne Collins", "Veronica Roth"],
  answer: "J.K. Rowling",
  },
  {
  question: "Qual é o maior rio do mundo?",
  options: ["Mississippi", "Amazonas", "Nilo", "Yangtzé"],
  answer: "Nilo",
  },
  {
  question: "Qual é o nome do cientista que formulou a teoria da relatividade?",
  options: ["Albert Einstein", "Galileu Galilei", "Isaac Newton", "Thomas Edison"],
  answer: "Albert Einstein",
  },
  {
  question: "Qual foi o primeiro ser humano a pisar na Lua?",
  options: ["Yuri Gagarin", "Buzz Aldrin", "Neil Armstrong", "Alan Shepard"],
  answer: "Neil Armstrong",
  },
  {
  question: "Qual é a capital da Austrália?",
  options: ["Melbourne", "Sydney", "Canberra", "Brisbane"],
  answer: "Canberra",
  },
  {
  question: "Qual é o nome do primeiro presidente dos Estados Unidos?",
  options: ["George Washington", "Thomas Jefferson", "John Adams", "James Madison"],
  answer: "George Washington",
  },
  {
  question: "Qual é o maior animal terrestre?",
  options: ["Baleia-azul", "Elefante-africano", "Rinoceronte-branco", "Girafa"],
  answer: "Elefante-africano",
  },
  {
  question: "Qual é o nome da estrela mais próxima da Terra?",
  options: ["Próxima Centauri", "Sirius", "Betelgeuse", "Alfa Centauri"],
  answer: "Alfa Centauri",
  },
  {
    question: "Qual é o nome do ator que interpreta o personagem Homem de Ferro, nos filmes da Marvel?",
    options: ["Chris Hemsworth", "Robert Downey Jr.", "Mark Ruffalo", "Chris Evans"],
    answer: "Robert Downey Jr.",
    } 
];
  
  export default data;